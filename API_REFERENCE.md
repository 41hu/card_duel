...
- 被动钩子（MatchState 固定时机调用）：`on_turn_start`、`on_turn_end`、`on_attack_hit`、`on_taking_damage->int`、`on_heal->int`、`on_dot_damage(idx, dot_types)`、`on_attack_cast->int`、`is_immune(idx, effect)`
