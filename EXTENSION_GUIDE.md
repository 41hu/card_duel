...
3. **武器加成**：斩铁+2 / 突刺+1（移动贴脸额外+3）/ 贤者之书+2 / 共鸣+2（连击≥2）
...
## 14. 技能次数配置（角色数据表，协作者直接调）

主动技能次数由角色数据表字段控制（`scripts/data/character_data.gd`，无需改逻辑层）：

```gdscript
"hunter": {
    ...,
    "skill_turn_limit": 2,    # 每回合最多 2 次（默认 1）
    "skill_game_limit": 3,    # 整局最多 3 次（默认 -1 = 无限）
},
```

- 两个字段都省略 = 每回合 1 次、整局无限（现状）
- 达到每回合上限：技能按钮消失，再调返回"本回合已达次数上限"
- 达到整局上限：返回"整局已达上限(N次)"

---

## 15. 加多段攻击角色（预留机制，如快枪手）
