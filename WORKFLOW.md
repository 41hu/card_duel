...
> 2026-07-31 全面代码审查后更新（23 条）。标注 [UI] 的归主维护者（平衡性开发者不处理）；无标注的为 `scripts/core/` 逻辑问题，归主维护者修复，平衡性开发者改动相关代码时注意别引入相同问题。

### 严重（崩溃/卡死）

1. **[UI] 结算界面永远空白** — `battle_ui.gd:561` 切场景时丢弃结果，`settlement_ui.gd:10` 在 `_ready` 才 connect，`game_ended` 信号已同步发完
2. **[UI] 对战中断线无反馈** — `network.gd:45-86` 对方掉线后 UI 静默冻结，只能强退
3. **[UI] 武器选择弹窗无超时** — `match_state.gd:420-433` 忽略弹窗后 `pending_weapon_id` 残留，下一回合会装备上一回合的武器
4. **陷阱双重扣血** — `movement_system.gd:116` 内部扣 HP，`match_state.gd:384`、`character_skills.gd:232`（刺客位移）又各扣一次 → 踩陷阱实际扣 6 血
5. **陷阱致死无死亡判定** — 推人/吸引/威慑触发陷阱后不走复活流程，HP 可为负

### 逻辑错误

6. **冻结每局只生效一次** — `status_system.gd:46-49` `frozen_lockout` 跳过回合时置 true，第二次冻结被静默拒绝
7. **BP 先手失效** — `match_state.gd:90-91` `init_match` 重新 `randi()`，无视 BP 定好的 `bp_first`
8. **被闪避仍消耗防具耐久** — `combat_system.gd:95-126` 声明攻击即扣耐久（违反"被闪避不消耗"规则）
9. **穿心卡恒能打出** — `combat_system.gd:46` `max(0, 面板-距离)+3` 恒 ≥3，规则要求"面板-距离≤0 无法打出"
10. **摧毁装备失败仍消耗卡** — `match_state.gd:396-398`
11. **响应窗口无独立超时** — `match_state.gd:502-513` 响应期超时状态冲突，迟到的响应可作用于下一回合
12. **三把武器效果未实现** — `combat_system.gd:85-92,159`：长弓"远程+1"丢失、连弩"牵制+2"缺失、突刺"贴脸+3"缺失
