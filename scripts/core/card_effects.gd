...
func _atk_freeze(player_idx: int, card: Dictionary):
	# 冻结冷却中（不能连续冻结）：卡不消耗、明确提示（快速模式/自定义「冻结无冷却」无冷却）
	var opp = _m.get_opponent(player_idx, int(card.get("target", -1)))
	if opp >= 0 and not (_m.rapid_mode or _m.freeze_no_cooldown) and _m.players[opp].frozen_lockout > 0:
...
func _item(player_idx: int, card: Dictionary):
	...
	# 蔓生种子（树妖）：道具卡只能叠加——目标格必须已有种子且未满 2 层（新种由蔓延技能负责）
	if item_type == "vine_seed":
		var layers = _m.item_system.get_seed_layers(pos)
		if layers <= 0:
			return {success=false, msg="请选择已有蔓生种子的地格叠加（新种用蔓延技能）"}
		if layers >= 2:
			return {success=false, msg="该格种子已达2层上限"}
