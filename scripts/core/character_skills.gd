...
func on_attack_hit(attacker_idx: int, defender_idx: int, damage: int, damage_type: int):
	var p = match.players[attacker_idx]
	match p.char_id:
...
func on_taking_damage(defender_idx: int, attacker_idx: int, base_damage: int) -> int:
	var p = match.players[defender_idx]
	match p.char_id:
...
func on_turn_end(player_idx: int):
	...
	if p.char_id == "warlock" and p.ap_function >= 1:
		match.card_systems[player_idx].draw_cards(1)
		match.add_log(player_idx, "术士+1抽")
...
func on_attack_cast(player_idx: int, type_id: String) -> int:
	...
	if type_id in ["magic", "chant"] and p.mage_buffed:
		bonus = 2
		p.mage_buffed = false
		match.add_log(player_idx, "法师强化: +2")
	return bonus
...
func armor_durability_bonus(player_idx: int) -> int:
	if match.players[player_idx].char_id == "paladin": return 1
	return 0
...
func _swordsman_hit(player_idx: int, damage_type: int):
	...
	if damage_type == Config.DamageType.PHYSICAL:
		...
		match.add_log(player_idx, "剑士+2HP")
...
func _berserker_rage(player_idx: int):
	match.status.add_buff(player_idx, "attack_up", 1, 2)
	match.add_log(player_idx, "狂战士+1近战")

func _mage_discard(player_idx: int) -> Dictionary:
	var cs = match.card_systems[player_idx]
	if cs.hand.is_empty(): return {success=false, msg="没有可弃的牌"}
	cs.random_discard(1)
	match.players[player_idx].mage_buffed = true
	match.add_log(player_idx, "弃牌强化:下次魔法+2")
	return {success=true}
...
func _assassin_move(player_idx: int, params: Dictionary) -> Dictionary:
	...
	if not match.movement.move_player(player_idx, dir): return {success=false, msg="无法移动"}
	var td = match.movement.check_trap_trigger(player_idx)
	if td > 0: match.players[player_idx].hp -= td
	match.add_log(player_idx, "暗影步")
	return {success=true}
