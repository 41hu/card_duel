...
const CHARACTER_DB = {
	...
	"rogue":       {name="盗贼",    hp=23, near=6, range=3, magic=1, skill="rogue",        skill_desc="妙手：你打出的「夺取」卡不消耗功能行动点。劫富：近战命中或受到攻击（被造成伤害）后，若来源手牌不少于你，随机夺取其1张（每回合限1次）；偷牌成功后进入潜行。潜行（被动）：潜行期间远程/法术攻击无法瞄准你（近战可命中，命中后现形）；你打出任何手牌即现形；在你触发来源角色的下一次回合开始时自然消失。济贫：每回合限1次，选一张自己的手牌送给任意目标（含4人局队友）并公示（喂肥目标使劫富更易触发）"},
}
