...
const CHARACTER_DB = {
	...
	"vine_ent":    {name="蔓生树妖", hp=25, near=3, range=3, magic=4, skill="vine_ent",     skill_desc="生根：每次移动或回合开始时，所在格自动长出「蔓生种子」（自身免疫其效果）。蔓延：道具卡变为「蔓生种子」，只能种在已有种子地格的相邻格，一步步向外铺开。神经毒素：踩到种子的单位获得1层「致残」（永久、可叠加）；带致残的单位每次位移（移动/被推拉）受2点真实伤害，并消去1层致残。摧残：你对带致残目标的攻击伤害+1"},
}
