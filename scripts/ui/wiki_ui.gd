...
· 校准（寻踪者）：远程/法术命中叠 1 层，每层远程伤害+1，永久持续可无限叠；远程/法术攻击未造成伤害（闪避/格挡牵制减到0/护甲免疫/无法打出）则全部清空\n· 吸引/威慑：强制对方位移 1 格（被吸引贴脸时你会后退腾位）\n· BP 禁选流程：先手禁 1 → 后手禁 1 → 先手选 → 后手选 → 随机先手开战")
	_add_characters()
