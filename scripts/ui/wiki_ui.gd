...
const CHAR_SKILLS: Dictionary = {
	...
	"vine_ent": [
		["被动技", "生根", "每次移动或回合开始时，所在格自动长出「蔓生种子」——脚下的种子不可被破坏，树妖自身免疫其效果。"],
		["被动技", "蔓延", "道具卡变为「蔓生种子」，只能种在已有种子地格的相邻格（生根种子亦可作为蔓延源；同格上限 1），一步步向外铺开。"],
		["被动技", "神经毒素", "踩到种子的单位获得 1 层「致残」（永久、可叠加）。带致残的单位每次位移（移动/被吸引/被威慑/被推等）受到 2 点真实伤害（无视护甲与防御减伤），并消去 1 层致残。"],
		["被动技", "摧残", "你对带有致残的单位攻击伤害 +1。"],
		["被动技", "除根（反制）", "其他角色（含蔓生树妖自身对他人种子）可以消耗 1 张「近战」或「重击」卡 + 1 攻击行动点，点击自己所在格或相邻格的地格道具进行除根，清除 1 层蔓生种子（不攻击角色、不触发响应）；蔓生树妖所在地格的种子扎根不可破坏；「摧毁」卡可选择任意地格的蔓生种子进行破坏（无距离限制）。"],
	],
}
