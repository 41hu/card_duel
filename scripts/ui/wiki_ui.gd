...
const CHAR_SKILLS: Dictionary = {
	...
	"warlock": [
		...
		["被动技", "枯萎", "自身受到的所有回血效果 −1（回复 +1 时实际回复 0）。"],
	],
	...
	"spellblade": [["主动技", "魔力引导", "装备近战武器时可用。选择并丢弃一张「魔法」攻击卡，消耗 1 攻击行动点，视作打出「近战」攻击，无视距离限制；或选择并丢弃一张「吟唱」攻击卡，消耗 2 攻击行动点，视作打出「重击」攻击，无视距离限制。可被格挡响应。回合不限次数。"]],
}
...
func _fill_cards():
	...
	_add_card_block("摧毁（×3）", [
		"效果：打出后三选一：① 盲丢手牌：随机丢弃对方 1 张手牌；② 摧毁装备：指定摧毁对方一件已装备的武器或防具（武器回到幻化池可再次生成，防具直接消失不回池）；③ 拆除道具：点击棋盘指定格子，拆除该格道具（默认一次拆 1 个；捕兽夹一次清除同格全部；部分道具类型免疫拆除）",
	...
	_add_card_block("武器牌（近战/远程/法术武器牌，各 ×2）", [
		"效果：打出后随机幻化一把该类型尚未生成的武器，确认后装备或丢弃（丢弃则回到幻化池）",
		"可交互：武器只能装备一件，装备新武器时旧武器回到幻化池；该类型武器已全部生成时，武器牌无效且不消耗；被摧毁的武器回到幻化池可再次生成",
	...
	_add_card_block("防具牌（近战/远程/法术防具，各 ×1）", [
		"效果：打出立即装备对应防具",
		"可交互：防具只能装备一件，装备新防具时旧防具直接消失；被摧毁后直接消失（不回池）；防具耐久规则：每次被对应类型攻击命中消耗 1 点耐久（满耐久首击完全免疫 → 之后减半 → 耗尽碎裂；被闪避或 0 伤害攻击不消耗；铸甲师被动使耐久上限 +1 为 4）",
	])
