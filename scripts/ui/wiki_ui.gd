...
const CHAR_SKILLS: Dictionary = {
	...
	"mage": [
		...
		["主动技", "幻影", "手牌持有魔法/吟唱卡时可用（每回合限一次）：弃置一张魔法卡获得 1 层幻影、弃置一张吟唱卡获得 2 层幻影。幻影存在期间，你被攻击时（每段攻击独立判定）有 50% 概率闪避——成功则该段攻击完全无效（护甲不消耗），并消耗 1 层幻影；失败则正常结算。幻影只持续本回合，回合结束全部消失。"],
	],
...
func _fill_cards():
	...
	_add_card_block("武器牌（近战/远程/法术武器牌，各 ×2）", [
		...
		"武器一览（15 把，\"命中\" = 打出对应攻击类别的攻击卡且对目标造成伤害）：",
		"近战武器：",
		"· 斩铁：近战值+2",
