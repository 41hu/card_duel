...
const CHAR_SKILLS: Dictionary = {
	...
	"vine_ent": [
		["被动技", "生根", "每次移动或回合开始时，所在格自动长出「蔓生种子」——树妖自身免疫其效果。"],
		...
		["被动技", "除根（反制）", "其他角色（含蔓生树妖自身对他人种子）可以消耗 1 张「近战」或「重击」卡 + 1 攻击行动点，点击自己所在格或相邻格的地格道具进行除根，清除 1 层蔓生种子（不攻击角色、不触发响应）；「摧毁」卡可选择任意地格的蔓生种子进行破坏（无距离限制）。"],
	],
}
