...
const CHAR_SKILLS: Dictionary = {
	...
	"vine_ent": [
		["被动技", "生根", "蔓生树妖所在地格没有「蔓生种子」时立即获得 1 层（位移后/回合开始触发；该格种子不可被破坏；树妖自身无视种子效果）。"],
		["被动技", "蔓延", "道具卡锁定为「蔓生种子」，只能放置在已有蔓生种子地格的邻近格（生根种子亦可作为蔓延源；同格上限 1）。"],
		["被动技", "神经毒素", "单位到达蔓生种子所在格 → 获得 1 层「致残」（永久、可叠加）。致残单位每次位移（移动/被吸引/被威慑/被推等）受到 2 点真实伤害（无视护甲/防御减伤，计入 DoT 统计），并掉落 1 层致残。"],
		["被动技", "摧残", "你对带有致残的单位攻击伤害 +1。"],
		["反制", "除根", "其他角色（含蔓生树妖自身对他人种子）可以消耗 1 张「近战」或「重击」卡 + 1 攻击行动点，点击自己所在格或相邻格的地格道具进行除根，清除 1 层蔓生种子（不攻击角色、不触发响应）；蔓生树妖所在地格的种子扎根不可破坏；「摧毁」卡拆除道具时，蔓生种子仅限所在格/相邻格可选。"],
	],
}
...
func _fill_cards():
	...
	_add_card_block("武器牌（近战/远程/法术武器牌，各 ×2）", [
		...
		"武器一览（16 把，\"命中\" = 打出对应攻击类别的攻击卡且对目标造成伤害）：",
		"近战武器：",
		"· 斩铁：近战值+2",
