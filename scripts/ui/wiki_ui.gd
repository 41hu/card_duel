...
const CHAR_SKILLS: Dictionary = {
	...
	"hunter": [["主动技", "埋伏", "手牌持有远程攻击牌时可用。选择并丢弃一张「远程」攻击卡，不消耗攻击行动点，可放置 1 个捕兽夹；或选择并丢弃一张「穿心」攻击卡，同样不消耗攻击行动点，可放置 2 个捕兽夹。捕兽夹须放置于棋盘无单位空格，可重叠放置；首个到达该格的单位每个夹子受到 3 点伤害，触发后销毁。每回合限一次。"], ["被动技", "猎手本能", "猎人无视捕兽夹：踩上自己或其他猎人放置的捕兽夹时不受伤，夹子保留在原格继续生效。"]],
...
func _fill_cards():
	...
	_add_card_block("摧毁（×3）", [
		"效果：打出后三选一：① 盲丢手牌：随机丢弃对方 1 张手牌；② 摧毁装备：指定摧毁对方一件已装备的武器或防具（武器回到幻化池可再次生成，防具直接消失不回池；饲甲人的活铠免疫摧毁，此时摧毁卡不消耗）；③ 拆除道具：点击棋盘指定格子，同格所有可摧毁道具类型同时按各自规则拆除（默认类拆 1 个；可堆叠类如捕兽夹一次清空同格全部；免疫类跳过不影响其他类型）",
	...
	_add_card_block("防具牌（近战/远程/法术防具，各 ×1）", [
		"效果：打出立即装备对应防具",
		"可交互：防具只能装备一件，装备新防具时旧防具直接消失；被摧毁后直接消失（不回池）；防具耐久规则：每次被对应类型攻击命中消耗 1 点耐久（满耐久首击完全免疫 → 之后减半 → 耗尽碎裂；被闪避或 0 伤害攻击不消耗；强化攻击命中额外 −2 耐久且不叠加正常消耗；铸甲师被动使耐久上限 +1 为 4）。饲甲人例外：无法装备防具卡，打出改为修复活铠（耐久回满；活铠满耐久时打出无效果）；活铠免疫摧毁",
	])
